...
# Specify the Python executable where RDKit is installed via Anaconda
# Replace "C:\\your\\path\\to\\python.exe" with the actual path on your system
# Importing 'rdkit' modules for chemoinformatics, prediction model and scaler
use_python("path\\to\\python.exe")
rdkit <- import("rdkit")
Chem <- rdkit$Chem
Descriptors <- rdkit$Chem$Descriptors
AllChem <- rdkit$Chem$AllChem
best_model <- readRDS(file = "model.rds")      
scaling_model <- readRDS(file = "scaling_model.rds")
...
colnames(maccs_df) <- paste0("MACCS", 1:length(maccs_df))
...
# Removing SMILES notations from the dataframe
descriptors_dataframe_scaled <- select(descriptors_dataframe,
                                       -SMILES)

# Converting the dataframe into numeric type
descriptors_dataframe_scaled <- as.data.frame(lapply(descriptors_dataframe_scaled, 
                                                     as.numeric))

# Performing the process of scaling descriptors to the range between 0 and 1
descriptors_dataframe_scaled <- predict(scaling_model,
                                        descriptors_dataframe_scaled)

# Saving MACCS Keys as dataframe, combining scaled descriptors with them and converting the whole dataframe into 'numeric' type
maccs_df <- as.data.frame(maccs_df)
descriptors_dataframe_scaled_all <- cbind(descriptors_dataframe_scaled, 
                                          maccs_df)
predict_data <- as.data.frame(lapply(descriptors_dataframe_scaled_all, 
                                     as.numeric))

# Performing prediction of HC50
HC50.calculated <- predict(best_model,
                           predict_data)

# Calculating the logHC50 to an actual concentration
HC50.log <- (10^HC50.calculated)

# Printing results
print(paste0("logHC50 = ", 
             HC50.calculated))
print(paste0("HC50 = ", 
             HC50.log, 
             " mg/L"))
